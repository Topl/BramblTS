...
export class Bip32Ed25519CkdTestVector {
  ...
  static fromJson(vector: any): Bip32Ed25519CkdTestVector {
    const input = vector.inputs;
    const output = vector.outputs;

    const path = input.path.map((x: [string, number]) => {
      const [type, index] = x;
      if (type === 'soft') {
    ...
    const childSK = new ExtendedEd25519Initializer(new ExtendedEd25519()).fromBytes(
      Buffer.from(output.childSecretKey, 'hex'),
    );

    const childVk = new PublicKey(Buffer.from(output.childVerificationKey, 'hex'));
...
export const bip32Ed25519CkdTestVectors = [
  {
  ...
  {
    description: 'test vector 5 - Derive the correct child keys at path ` m/0`/100` ` given a root extended secret key',
    inputs: {
      rootSecretKey:
        'f0d0f18e6ab029166fe4e89519ab64f42aa870fc2791fc472840c3a1ba507347fee30dcae1ae3941bde71e9ddd19eef33d0a7b91aaa4137cea6ef4ea3c27f96a1189e5ec0628974ed7846b594ed0ee2d3ef2d8f5b91d1860ffb0a065159df8be',
      path: [
