import { ImmutableBytes, Lock, Lock_Predicate, Lock_Commitment, Lock_Image, AccumulatorRootId, Event_GroupPolicy, SeriesId, TransactionOutputAddress, TransactionId, Event_SeriesPolicy } from "./types.js";
...
export class ContainsImmutable {
    ...
    static accumulatorRoot32Identifier(id: AccumulatorRootId): ContainsImmutable {
        return ContainsImmutable.addContainsImmutable(ContainsImmutable.string(Identifier.accumulatorRoot32), new ContainsImmutable(new ImmutableBytes({ value: id.value })));
    }

    static predicateLock(predicate: Lock_Predicate): ContainsImmutable {
        return ContainsImmutable.addContainsImmutable(ContainsImmutable.int(predicate.threshold), ContainsImmutable.list(predicate.challenges));
    }

    static imageLock(image: Lock_Image): ContainsImmutable {
        return ContainsImmutable.addContainsImmutable(ContainsImmutable.int(image.threshold), ContainsImmutable.list(image.leaves));
    }
    ...
    static commitmentLock(commitment: Lock_Commitment): ContainsImmutable {
        ...
        const partiallyMergedContainsImmutable = ContainsImmutable.addContainsImmutable(thresholdContainsImmutable, lengthContainsImmutable);
    ...
    static seriesIdValue(sid: SeriesId) {
        return ContainsImmutable.addContainsImmutable(ContainsImmutable.string(Identifier.series32), new ContainsImmutable(new ImmutableBytes({value: sid.value})));
    }

    static transactionIdentifier(id: TransactionId) {
        return ContainsImmutable.addContainsImmutable(ContainsImmutable.string(Identifier.ioTransaction32), new ContainsImmutable(new ImmutableBytes({value: id.value})));
    }

    static transactionOutputAddress(v: TransactionOutputAddress) {
        const networkAndLedgerContainsImmutable = ContainsImmutable.addContainsImmutable(ContainsImmutable.int(v.network), ContainsImmutable.int(v.ledger));
        const combinedIntContainsImmutable = ContainsImmutable.addContainsImmutable(networkAndLedgerContainsImmutable,ContainsImmutable.int(v.index));
        return ContainsImmutable.addContainsImmutable(combinedIntContainsImmutable,ContainsImmutable.transactionIdentifier(v.id));
    }



    static groupPolicyEvent(groupPolicy: Event_GroupPolicy) {
        const partiallyCombinedContainsImmutable = ContainsImmutable.addContainsImmutable(ContainsImmutable.string(groupPolicy.label), ContainsImmutable.seriesIdValue(groupPolicy.fixedSeries));
        return ContainsImmutable.addContainsImmutable(partiallyCombinedContainsImmutable, ContainsImmutable.transactionOutputAddress(groupPolicy.registrationUtxo));
    }
    ...
    static seriesPolicyEvent(seriesPolicy: Event_SeriesPolicy) {
        const combinedIntContainsImmutable = ContainsImmutable.addContainsImmutable(ContainsImmutable.string(seriesPolicy.label),ContainsImmutable.int(seriesPolicy.tokenSupply.value));
        const combinedIntContainsImmutable2 = ContainsImmutable.addContainsImmutable(combinedIntContainsImmutable,ContainsImmutable.transactionOutputAddress(seriesPolicy.registrationUtxo));
        const combinedIntContainsImmutable3 = ContainsImmutable.addContainsImmutable(combinedIntContainsImmutable2,ContainsImmutable.fungibility(seriesPolicy.fungibility));
        return ContainsImmutable.addContainsImmutable(combinedIntContainsImmutable3, ContainsImmutable.quantityDescriptor(seriesPolicy.quantityDescriptor));
    }
