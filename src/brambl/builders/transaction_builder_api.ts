...
export class transactionBuilderApiImpl implements TransactionBuilderApi {
    ...
    lvlOutput(predicate: Lock_Predicate, amount: Int128): UnspentTransactionOutput {
        return new UnspentTransactionOutput({
            address: new LockAddress({
                ...
                ledger: this.ledgerId,
                id: new LockId({
                    value: ContainsEvidence.blake2bEvidenceFromImmutable(new Lock({ predicate: predicate })).evidence.digest.value
                })
            }),
    ...
    unprovenAttestation(predicate: Lock_Predicate): Attestation {
        return new Attestation({ predicate: new Attestation.Predicate({ lock: predicate, responses: Array.from({ length: predicate.challenges.length }, () => new Proof()) }) })
    }
    ...
    buildSimpleLvlTransaction(lvlTxos: Txo[], lockPredicateFrom: Lock_Predicate,
        ...
        const lvlOutputForChange = this.lvlOutput(lockPredicateForChange, new Int128({ value: new Uint8Array(encodedDataForChange) }));
        const encodedDataForRecipient = textEncoder.encode(amount.toString());
        const lvlOutputForRecipient = this.lvlOutputWithLockAddress(recipientLockAddress, new Int128({ value: new Uint8Array(encodedDataForRecipient) }));
        ...
        ioTransaction.outputs = totalValues - BigInt(amount) > BigInt(0)
    ...
    buildSimpleGroupMintingTransaction(
        ...
        if (isLeft(validationResult)) {
            return left(new UnableToBuildTransaction(
        ...
        const d = this.datum();
        const utxoMinted = this.groupOutput(mintedConstructorLockAddress, quantityToMint, GroupPolicySyntax.computeId(groupPolicy));
        return right(new IoTransaction(
            {
                inputs: [
    ...
    buildSimpleSeriesMintingTransaction(registrationTxo: Txo, registrationLock: Lock_Predicate, seriesPolicy: Event_SeriesPolicy, quantityToMint: Int128, mintedConstructorLockAddress: LockAddress): Either<BuilderError, IoTransaction> {
        const lock = new Lock();
        lock.predicate = registrationLock;
        const registrationLockAddr = new LockAddress({
            network: this.networkId,
    ...
    validateConstructorMintingParams(
        ...
        } else if (AddressCodecs.uint8ArrayToNumber(quantityToMint.value) < 0 || AddressCodecs.uint8ArrayToNumber(quantityToMint.value) === 0) {
