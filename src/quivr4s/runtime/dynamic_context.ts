...
export class DynamicContext {
    ...
    digestVerify(routine: string, verification: DigestVerification): QuivrResult<DigestVerification> {
        ...
        if (verifier === null) return either.left(ValidationError.failedToFindDigestVerifier({ name: 'DynamicContext', message: `failed to find digest verifier for ${routine}` }));
    ...
    signatureVerify(routine: string, verification: SignatureVerification): QuivrResult<SignatureVerification> {
        ...
        if (verifier === null) return either.left(ValidationError.failedToFindSignatureVerifier({ name: 'DynamicContext', message: `failed to find signature verifier for ${routine}` }));
    ...
    useInterface(label: string): QuivrResult<Data> {
        ...
        if (interfaceObj === null) return either.left(ValidationError.failedToFindInterface({ name: 'DynamicContext', message: `failed to find interface for ${label}` }));
